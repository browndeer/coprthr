...
#define __writefile(file,filesz,pfile) \
	__check_error( writefile( file,filesz,pfile ) )

static int writefile( char* file, size_t filesz, char* pfile )
{
...
#define __writefile_cpp(file,filesz,pfile) \
	__check_error( writefile_cpp(file,filesz,pfile) )

static int writefile_cpp( char* file, size_t filesz, char* pfile )
{
	FILE* fp = fopen(file,"w");
...
//#endif
   ...
   (*log)[0] = '\0';
	...
	char* tmplog = (char*)malloc(DEFAULT_BUF2_SZ);
	tmplog[0] = '\0';

	char* cmd = (char*)malloc(DEFAULT_CMD_SZ);
...
*/
		...
		p2_prev = p2;
		__execshell(buf1,p2);
		if (p2 != p2_prev) append_str(log,p2_prev,0,0);
		printcl( CL_DEBUG "execshell ] %s",buf1);

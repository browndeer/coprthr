 ...
 * XXX I disagree - modified for distrib device memory allocation -DAR 
 */
#include <sys/types.h>
#include <sys/mman.h>
#include <errno.h>
...
//void* devmembase = 0;
static void* devmemlo = 0;
static void* devmemhi = 0;
//void* devmem_break = (void*)DEVMEMLO;
static void* devmem_break = 0;

void dmalloc_init( void* lo, void* hi )
{
	devmem_break = devmemlo = lo;
	devmemhi = hi;
}
...
static int dbrk( int devnum, const void* addr )
{
	if (addr < (void*)DEVMEMLO || addr > (void*)DEVMEMHI) return -1;
	devmem_break = (void*)addr;
	return 0;
}

static void* dsbrk( int devnum, intptr_t incr )
{
	void* addr = devmem_break + incr;
	if (addr < (void*)DEVMEMLO || addr > (void*)DEVMEMHI) return (void*)-1;
	void* b = devmem_break;
	devmem_break = addr;
	return b;
}
...
static void *imalloc(int devnum, size_t size);
static void ifree(int devnum, void *ptr);
static void *irealloc(int devnum, void *ptr, size_t size);
 ...
 */
static void *
map_pages(int devnum,size_t pages)
{
    ...
    if ((result = dsbrk(devnum,bytes)) == (void *)-1)
	return NULL;
	...
	if (dsbrk(devnum,(intptr_t)(rresult - result)) == (void *) -1) {
	    /* we failed, put everything back */
	    if (dbrk(devnum,result)) {
...
//    malloc_origo = pageround((size_t)(uintptr_t)sbrk((intptr_t)0))
    malloc_origo = pageround((size_t)(uintptr_t)dsbrk(0,(intptr_t)0))
	>> malloc_pageshift;
 ...
 */
static void *
malloc_pages(int devnum, size_t size)
{
	...
	if ((void*)pf->page >= (void*)dsbrk(devnum,0))
...
//	    memset(p, SOME_JUNK, size << malloc_pageshift);
		wrtwarning("XXX not filling alloc with junk!\n");
    }
...
static inline int
malloc_make_chunks(int devnum, int bits)
{
    struct  pginfo *bp;
    void *pp;
    int i, k;
    ...
    pp = malloc_pages(devnum, malloc_pagesize);
    if (pp == NULL)
	return 0;
 ...
 */
static void *
malloc_bytes(int devnum, size_t size)
{
    size_t i;
    int j;
    u_int u;
    ...
    if (page_dir[j] == NULL && !malloc_make_chunks(devnum, j))
	return NULL;
 ...
 */
static void *
imalloc(int devnum, size_t size)
{
    void *result;
	...
	result = malloc_bytes(devnum, size);
    else
	result = malloc_pages(devnum, size);
...
//	memset(result, 0, size);
	wrtwarning("XXX not filling alloc with zeros!\n");
 ...
 */
static void *
irealloc(int devnum, void *ptr, size_t size)
{
    void *p;
    size_t osize, idx;
    struct pginfo **mp;
...
//	    memcpy(p, ptr, osize);
		wrtwarning("XXX realloc not copying contents!\n");
	else
//	    memcpy(p, ptr, size);
		wrtwarning("XXX realloc not copying contents!\n");
	ifree(devnum,ptr);
...
static inline void
free_pages(int devnum, void *ptr, size_t idx, struct pginfo *info)
{
    size_t i;
...
//	madvise(ptr, l, MADV_FREE);
	wrtwarning("XXX skipping madvise() call, no device equivalent!");

    tail = (char *)ptr+l;

    /* add to free-list */
    if (px == NULL)
	px = host_malloc(sizeof *px);	/* This cannot fail... */
    px->page = ptr;
    px->end =  tail;
    px->size = l;
    ...
    if (pf->next == NULL &&			/* If we're the last one, */
      ...
      malloc_brk == dsbrk(devnum,(intptr_t)0)) {	/* ..and it's OK to do... */
...
static inline void
free_bytes(int devnum, void *ptr, size_t idx, struct pginfo *info)
{
    size_t i;
    struct pginfo **mp;
    void *vp;
...
ifree(int devnum, void *ptr)
{
    ...
    if (info < MALLOC_MAGIC)
        free_pages(devnum, ptr, idx, info);
    else
	free_bytes(devnum, ptr, idx, info);
...
static void *
pubrealloc( int devnum, void *ptr, size_t size, const char *func)
{
    void *r;
    int err = 0;
	...
	r = imalloc(devnum, size);
	err = (r == NULL);
    } else {
        r = irealloc(devnum, ptr, size);
	err = (r == NULL);
    }
    UTRACE(ptr, size, r);
    malloc_active = 0;
...
int
dposix_memalign(int devnum, void **memptr, size_t alignment, size_t size)
{
    int err;
    void *result;
    ...
    result = pubrealloc(devnum, NULL, (size & alignment),
		" in dposix_memalign()");
    errno = err;
...
void *
dcalloc(int devnum, size_t num, size_t size)
{
    void *ret;
    ...
    ret = pubrealloc(devnum, NULL, num * size, " in dcalloc():");

    if (ret != NULL)
//	memset(ret, 0, num * size);
		wrtwarning("XXX no filling alloc with zeros!\n");

    return ret;
}

void
dfree(int devnum, void *ptr)
{

    pubrealloc(devnum, ptr, 0, " in dfree():");
}

void *
drealloc(int devnum, void *ptr, size_t size)
{

    return pubrealloc(devnum, ptr, size, " in drealloc():");
}

...
printcl( CL_DEBUG "prg->bin_sz[devnum]=%d",prg->bin_sz[devnum]);
...
//	#endif
	...
	if (prg->imp.clsymtab == 0) {
		...
		prg->imp.info = malloc( sizeof(struct program_info_struct) );

		prg->imp.ksu = (size_t*)malloc(prg->imp.nkrn*sizeof(size_t));
		...
		for(i=0;i<clsymtab_n;i++) {
			...
			prg->imp.ksu[i] = prg->imp.clsymtab[i].e_su;

			printcl( CL_DEBUG "prg->imp.ksu[%d] set to %d",i,prg->imp.ksu[i]);
	...
	printcl( CL_DEBUG "_program_info %p 0x%x %d",proginfo,
		*(unsigned int*)proginfo, *(((unsigned int*)proginfo)+1));

	memcpy(prg->imp.info,proginfo,sizeof(struct program_info_struct));
	...
	printcl( CL_DEBUG "srec srec_sz %p %d\n",srec,srec_sz);
...
//		strncpy(prg->imp.v_kbin_tmpfile[devnum],tmpfile,32);
		...
		prg->imp.v_kbin[devnum] = srec;
		prg->imp.v_kbin_tmpfile[devnum] = (char*)malloc(32); /* XXX */
		strncpy(prg->imp.v_kbin_tmpfile[devnum],srectmpfile,32);
...
//	struct _elf_data* edata = (struct _elf_data*)comp(
	int err = comp( devid, prg->src,prg->src_sz, &prg->bin[devnum],
		&prg->bin_sz[devnum], prg->build_options[devnum],&prg->build_log[devnum]);
...
int bind_ksyms_elf( struct _imp_ksyms_struct* ksyms, void* h, char* kname )
{

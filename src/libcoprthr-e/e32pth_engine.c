...
int e32pth_engine_ready() 
{ return((engine_is_started)? 1:0); }

//static void* e32_engine( void* p );

void* e32pth_engine_startup( void* p )
{
	int i;
...
int e32pth_engine_klaunch( int engid_base, int ne, struct workp* wp,
	struct cmdcall_arg* argp )
{
   ...
   if (nthr > E32_NCORES) {
	...
	if (argp->k.krn->imp.ksu > E32_CORE_LOCAL_MEM_HI - 0x200 ) {
		printcl( CL_ERR "exceeded maximum stack size");
		return(CL_OUT_OF_RESOURCES);
	}

//	size_t stack_base = 0x7800 - argp->k.krn->imp.ksu;
	size_t stack_base = E32_CORE_LOCAL_MEM_HI - 0x200 - argp->k.krn->imp.ksu;
//	if (argp->k.krn->imp.ksu > 0x800) {
	if ( stack_base < proginfo->core_local_data + 0xb0 ) {
		printcl( CL_ERR "exceeded maximum stack size");
		return(CL_OUT_OF_RESOURCES);
	}
//	printcl( CL_WARNING "stack usage check disabled" );
...
//	size_t local_mem_free = proginfo->core_local_data + 0xb0;
...
//            else if (sz & 8) align = 8;
            ...
            if (sz > local_mem_sz) {
               ...
               *(void**)p = (void*)(local_mem_free|E32_CORE0_LOCAL_MEM_BASE);
               local_mem_free += sz;
               local_mem_sz -= sz;
...
//   }
	...
	for (corenum=0; corenum<E32_NCORES;  corenum++) 
   	core_control_data.run[corenum] = 77;
	e32_write_ctrl_run(core_control_data.run);

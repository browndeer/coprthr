...
int e32sl_engine_ready() 
{ return((engine_is_started)? 1:0); }


void* e32sl_engine_startup( void* p )
{
	int i;
...
int
e32sl_engine_klaunch( int engid_base, int ne, struct workp* wp,
	struct cmdcall_arg* argp )
{
	...
	e32_ptr_t device_mem_lo = E32_DRAM_BASE;
...
//	local_mem_free = 0x7200;
//	size_t local_mem_sz = 0x6000 - local_mem_free;
	size_t local_mem_sz = THR_STACK_MEMHI - nthr*THR_STACK_SZ - local_mem_free;
...
//	unsigned int narg =  argp->k.krn->narg;
	...
	/* XXX now we have the info we need to calculate local mem start
	 * XXX which is great since we needed it at the top of the routine! -DAR 
	 */

...
#define CHECK_ERROR( call ) do { \
	int rc; \
	if ( rc=call) { \
		fprintf(stderr,"clinfo: ERROR %d reported from '%s'\n",rc,#call); \
		exit(1); \
	} } while(0)
...
int main() 
{
	printf("clinfo: report OpenCL platform and device information\n");
	...
	if (!nplatforms) {
		printf("clinfo: No platforms found\n");
		exit(0);
	} else {
		printf("clinfo: Number of platforms found = %d\n",nplatforms);
	}
	...
	for(i=0;i<nplatforms;i++) {
		...
		if (ndev) {
			...
			printf("clinfo: Number of devices found for this platform = %d\n",
				ndev);
			...
			printf("clinfo: No devices found for this platform\n");

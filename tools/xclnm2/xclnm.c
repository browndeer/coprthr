...
struct type_entry type_table[] = {
	...
	{ TYPE_UINT8, 1, "unsigned char", "uchar" },
	{ TYPE_UINT16, 2, "unsigned short", "ushort" },
	{ TYPE_UINT32, 4, "unsigned int", "uint" },
	{ TYPE_UINT64, 8, "unsigned long", "ulong" },
			...
			case NTYP_FUNC:
				...
				if (nptr1->n_func.flags & F_FUNC_DEF) {
					...
					for(i=0,nptr2=nptr1->n_func.args;i<n;i++,nptr2=nptr2->next) {
						if (i>0) fprintf(fp,", ");	
						fprintf_type(fp,nptr2->n_arg.argtype);
					}

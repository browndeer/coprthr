...
//
// This program is based on llvm-extract and modified to allow the
// extraction of multiple functions.

/* DAR */

//===- llvm-ex.cpp - LLVM function extraction utility ----------------===//
//
...
int main(int argc, char **argv) {
  ...
  // Figure out which function(s) we should extract
	std::vector<std::string> vfunc;
	std::string stmp = ExtractFunc;
	size_t si=0,sj;
	while((sj=stmp.find_first_of(',',si))!=std::string::npos) {
		vfunc.push_back(stmp.substr(si,sj-si));
		si = sj+1;
	}
	vfunc.push_back(stmp.substr(si,stmp.size()-si));

	std::vector<GlobalValue*> GVs;
	for(int i=0; i<(int)vfunc.size(); i++) {
		Function* F = M.get()->getFunction(vfunc[i]);
